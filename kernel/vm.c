...
extern char etext[]; // kernel.ld sets this to end of kernel code.acctually is a paddress
...
//initialize a kernelpagetable
pagetable_t uvm_kpt_init(struct proc *p)
{
...
walk(pagetable_t pagetable, uint64 va, int alloc)
{
  if (va >= MAXVA)
    panic("walk");
  //get page start addr level by level
...
int mappages(pagetable_t pagetable, uint64 va, uint64 size, uint64 pa, int perm) //perm means permission
{
  uint64 a, last;
  pte_t *pte;
...
uvmalloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  ...
  for (a = oldsz; a < newsz; a += PGSIZE)
  {
    ...
    {
      kfree(mem);
      uvmdealloc(pagetable, a, oldsz); //Is this low efficiency?
      return 0;
    }
  }
  return newsz;
...
// process size.  Returns the new process size.
uint64
uvmkdealloc(pagetable_t pagetable, uint64 oldsz, uint64 newsz)
{
  if (newsz >= oldsz)
    return oldsz;
  ...
  {
    int npages = (PGROUNDUP(oldsz) - PGROUNDUP(newsz)) / PGSIZE;
    uvmunmap(pagetable, PGROUNDUP(newsz), npages, 0); //here differ from original version
  }
...
void freewalk(pagetable_t pagetable)
{
  ...
  for (int i = 0; i < 512; i++)
  {
    ...
    if ((pte & PTE_V) && (pte & (PTE_R | PTE_W | PTE_X)) == 0)
    {
      ...
      freewalk((pagetable_t)child); //post order travesal
      pagetable[i] = 0;             //why zero? kfree will set all elements to 1.
    }
    else if (pte & PTE_V)
    {
      panic("freewalk: leaf");
  ...
  /* why not kfree child pagetable? notice that:
  this is a recursive structure,all children will 
  also be freed. */
}
...
//copy user pagetable entry to kpagetable,
//except physical memory.
int uvmcopy2(pagetable_t ptb, pagetable_t kptb, uint64 oldsz, uint64 newsz)
{
  if (oldsz > newsz)
    return -1;
  pte_t *pte;
  ...
  {
    if ((pte = walk(kptb, va, 1)) == 0)
      return -1;
    *pte = *walk(ptb, va, 0) & (~PTE_U);
  }
  return 0;
}
